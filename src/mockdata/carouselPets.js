export const carouselPets = [
  {
    "pet": {
      images: [
        'https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/721A675B-1306-4A89-BE82-5F49B7B403E8%2F0?alt=media&token=59e950ba-cd18-47a0-90cc-99b4b42019e9'
      ],
      timestamp: {
        seconds: 1601499600,
        nanoseconds: 0
      },
      race: 'חתול',
      suitables: ' ילדים, מבוגרים, דירה',
      vaccinated: '0',
      type: '1',
      age: 0.1,
      number: '0525232664',
      userID: 'LYSODSd1TKdKOOuYQmYUxTNcifh1',
      goodWords: 'חמודים קטנים',
      poopTrained: '0',
      size: '0',
      region: '0',
      gender: '0',
      name: 'שני גורים קטנים',
      desc: 'אומנה דחופה מ...עכשיו!!! אפילו זמנית לשבוע הם באשקלון כרגע 🙏🙏🐱אח ואחות, בקבוק ואוכלים רך.',
      isAdopted: false
    },
    "position": 0
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F1?alt=media&token=be3d2771-b26b-4144-ac2e-5c24074f401f",
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "2",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 1
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "1",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 2
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "1",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 3
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "1",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 4
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "1",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 5
  },
  {
    "pet": {
      "size": 0,
      "name": "צמר",
      "type": 0,
      "goodWords": "גור חמוד למסירה",
      "number": "0503026730",
      "suitables": "ילדים, דירה, מבוגרים, בית עם חצר",
      "poopTrained": 0,
      "gender": 0,
      "images": [
        "https://firebasestorage.googleapis.com/v0/b/petadoption-a2cd3.appspot.com/o/5RM3F4kdVHQsK5yn3haoeNCBW0o1%2F2?alt=media&token=321476a3-3989-40eb-854f-e5009e40d7e7"
      ],
      "vaccinated": 0,
      "age": 0.1,
      "region": "1",
      "desc": "כלב רועים מעורב למסירה פרטים בפרטי",
      "race": "כלב רועים מעורב"
    },
    "position": 6
  }
]


